...
def compare_logs(
    ...
    *,
    ...
    progress receives (0..100 percent, status message).
    """
    _report(progress, 0.0, "準備中…")
    ...
    _report(progress, 10.0, "正規化完成")

    _report(progress, 15.0, "對齊中…")
    matcher = SequenceMatcher(a=left_norm, b=right_norm, autojunk=False)
    opcodes = matcher.get_opcodes()
    _report(progress, 40.0, "對齊完成，開始標色")
    ...
    for op_index, (tag, i1, i2, j1, j2) in enumerate(opcodes):
        ...
        pct = 40.0 + 55.0 * (op_index + 1) / total
        _report(progress, pct, f"標色 {op_index + 1}/{total}")

    _report(progress, 100.0, "完成")
